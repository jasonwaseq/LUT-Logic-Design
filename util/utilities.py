...
def runner(simulator, timescale, tbpath, params, defs=[], testname=None, pymodule=None, jsonpath=None, jsonname="filelist.json", root=None):
    ...
    dirparams = params.copy()
    # Icarus bug...
    if("filename_p" in dirparams.keys()):
        del dirparams["filename_p"]
    work_dir = os.path.join(tbpath, "run", testdir, get_param_string(dirparams), simulator)
    build_dir = os.path.join(tbpath, "build", get_param_string(dirparams))
